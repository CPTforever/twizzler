...
pub(crate) fn tls_variant1() -> Option<(usize, *mut u8, usize, usize)> {
    unsafe {
        let tls_template = TLS_INFO.wait(); 
        ...
        Some((tcb_base as usize, tcb_base, layout.size(), layout.align()))
    }
}
...
pub(crate) fn tls_variant2() -> Option<(usize, *mut u8, usize, usize)> {
    unsafe {
        ...
        *(mem as *mut u64) = mem as u64;
        Some((mem as usize, tls, layout.size(), layout.align()))
